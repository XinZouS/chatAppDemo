...
class ChatLogMenuLuncher : NSObject, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        menuViewDismissForSelecting(item: menuItems[indexPath.item])
    }
    ...
    func menuViewShowup(){
        ...
        blackBackground.addGestureRecognizer( UITapGestureRecognizer(target: self, action: #selector(menuViewDismissWithoutSelection)))
...
//        window.addSubview(collectionMenuView)
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1.2, options: .curveEaseOut, animations: {
//            self.collectionMenuView.frame = CGRect(x: 0, y: window.frame.height - height - dy!, width: window.frame.width, height: height)
            self.menuView.frame = CGRect(x: 0, y: window.frame.height - height - dy!, width: window.frame.width, height: height + dy!)
            self.blackBackground.alpha = 0.5
        }, completion: nil)
    }
    
    func menuViewDismissForSelecting(item: MenuItem?){
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1.2, options: .curveEaseIn, animations: {
...
//                self.collectionMenuView.frame = CGRect(x: 0, y: currWindow.frame.maxY, width: currWindow.frame.width, height: 0)
            ...
            case MenuString.Gifs:
                self.chatLogController?.showGifsLuncher()
            default:
                return
            }
        }
    }
    func menuViewDismissWithoutSelection(){
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1.2, options: .curveEaseIn, animations: {
            self.blackBackground.alpha = 0
            if let currWindow = UIApplication.shared.keyWindow {
//                self.collectionMenuView.frame = CGRect(x: 0, y: currWindow.frame.maxY, width: currWindow.frame.width, height: 0)
